                  ...
                  >
                    ...
                    {roomBookings.length > 0 ? (
                      <div className="text-[11px] text-slate-600 space-y-1.5 pt-1">
                        <div className="flex items-center justify-between gap-1 text-[10px] font-bold text-slate-500 uppercase tracking-wider flex-wrap">
                          <div className="flex items-center gap-1">
                            <Info className="w-3 h-3 text-slate-400" />
                            <span>Rincian Rapat Terjadwal pada Tanggal Ini:</span>
                          </div>
                          <div className="flex items-center gap-1.5 text-[11px] font-bold text-indigo-900 bg-indigo-50 px-2 py-0.5 rounded-md border border-indigo-200 normal-case">
                            <Clock className="w-3 h-3 text-indigo-600 shrink-0" />
                            <span>Rentang Waktu Pilihan: <strong className="font-mono text-indigo-950">{activeStartTimeStr} - {activeEndTimeStr}</strong> &bull; Durasi: <strong className="font-mono">{formatDuration(activeDuration)}</strong></span>
                          </div>
                        </div>
                      ...
                      <div className="text-[11px] text-emerald-800 flex flex-wrap items-center justify-between gap-2 p-2.5 rounded-xl bg-emerald-50/80 border border-emerald-200">
                        <div className="flex items-center gap-1.5 font-medium flex-wrap">
                          <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                          <span>
                            Ruangan ini kosong sepanjang hari (Rentang waktu: <strong className="font-semibold text-emerald-950">08:00 - 17:00</strong> &bull; Durasi: <strong className="font-semibold text-emerald-950">9 Jam</strong>). Sangat leluasa untuk digunakan!
                          </span>
                        </div>
                        <div className="flex items-center gap-1.5 text-[11px] font-bold text-emerald-950 bg-white px-2.5 py-1 rounded-lg border border-emerald-300 shadow-2xs shrink-0">
                          <Clock className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                          <span>Rentang Waktu Pilihan: <strong className="font-mono text-emerald-950">{activeStartTimeStr} - {activeEndTimeStr}</strong> &bull; Durasi: <strong className="font-mono">{formatDuration(activeDuration)}</strong></span>
                        </div>
                      </div>
                    )}
                  </div>
